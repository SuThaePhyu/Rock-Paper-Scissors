...
let menu = document.getElementById("cl");

function bt(){
    menu.style.visibility="visible";
    game.style.visibility="hidden";
    // p.style.textContent="hello"
}
function btn(){
...
function rock() {
  ...
  function result() {
    ...
    if (singleRound == '') {
      document.querySelector('.singleResult').textContent = playRound(
        playerResult,
        computerResult
      );
    } else {
    ...
    if (rounds == 5) {
      ...
      if (playerScore == computerScore) {
        document.querySelector('.winner').textContent =
          'THIS MATCH IS A TIE in 5 Rounds!';
      } else if (playerScore > computerScore) {
        document.querySelector('.winner').textContent =
          'CONGRATS YOU WIN in 5 Rounds!';
      } else {
        document.querySelector('.winner').textContent =
          'SORRY YOU LOSE in 5 Rounds !';
      }
    } else {
      return;
    }
  ...
  function kFun() {
    var para = document.querySelector('.k');
    if (para.textContent == '') {
      var a = document.createElement('a');
      var linkText = document.createTextNode('Play Again');
      a.appendChild(linkText);
      a.href = 'index.html';
      document.querySelector('.k').appendChild(a);
    } else {
